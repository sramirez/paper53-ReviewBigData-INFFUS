...
Given that previous scheme generates exact models derived from submodel fusion, it is presented as an ideal scenario for distributed learning. Nevertheless, this kind of fusion comes to be minority since creation and/or fusion of partial models is not possible in most of scenarios for several reasons. For instance, most of supervised methods demand to access to a large percentage of instances in the training set (usually, the whole set) in order to generalize adequately. In this case, submodels creation is no longer an option because of the underlying relationship between data partitions. 

Decision trees (DT) as supervised algorithms do not support partial models in the distributed environment. They are top-down algorithms that continuously evaluate splits by using statistics sketches computed on the entire dataset. In this case, all data partitions are involved on the statistics computation in every moment, but in the rare scenario where all feature values in a partition belongs to leaves nodes. Parallelization here is then considered as a more complex task to accomplish.

The main idea behind exact fusion is to utilize distributed data to make decisions regarding the construction of a single model allocated in the master node. In the case of DTs, statistics for each split can be computed using the distributed data, then the master node will decide locally which split is the best option for the current state of the model.

This scheme was originally implemented in the Parallel Learner for Assembling Numerous
Ensemble Trees (PLANET) method~\cite{Panda09-DT-RF}. In PLANET, master node controls the complete tree induction process, and launches the MR jobs that construct the nodes. It also maintains the tree model in memory, decides which split predicates will be evaluated, and eventually replicates the updated model across the nodes. Nodes to be evaluated are organized through several queues, one with nodes for MR-based evaluation and another for in-memory evaluation. Depending on the amount of instances involved in computations, nodes are pushed to one queue or the other one. We focus on MR evaluation which is more common.

As tree construction proceeds, the master node retrieves nodes from the queue, and schedules MR jobs to evaluate splits. Inside the MR jobs, the statistics for splits are computed in the map side by using the instances in the data partitions. Finally, the reduce side decides which split is more convenient for the tree model. Once a MR job ends, the master nodes updates the model with the nodes and the splits predicates selected, and updates the queues with new nodes at the periphery.
...
PLANET project does not only offer an elegant solution for single tree induction, but it also extends previous scheme to provide powerful exact algorithms based on bagging and boosting. Concerning bagging, PLANET allows to build multiple trees by maintaining a single queue of nodes for the whole set of trees. Construction of trees is then fully parallel. In boosting, PLANET constructs weak learners sequentially as usual. Residuals are easily computed since the model is sent to every MR job in full.
